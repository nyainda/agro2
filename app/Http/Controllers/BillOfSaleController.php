...
class BillOfSaleController extends Controller
{
    public function storeBillOfSale(Request $request, $animal_id)
    {
        ...
        return redirect()->route('Sale.showBillOfSale', ['animal_id' => $animal_id])
    ...
    public function showBillOfSale($animal_id)
    {
       ...
       // if ($animal->status === 'sold') {
            //return redirect()->route('index')->with('error', 'This animal has already been sold, and you cannot add BillOfSales.');
       // }
       $animals = AnimalContent::find($animal_id);
       // Get the associated BillOfSale ID
       $billOfSaleId = $animals ? $animals->getBillOfSaleId() : null;
        // Fetch BillOfSales related to the specified animal, ordered by created_at in descending order and paginate them
        $BillOfSales = BillOfSale::where('animal_id', $animal_id)
