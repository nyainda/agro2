...
class YieldRecordController extends Controller
{
    public function storeyieldrecord(Request $request, $animal_id)
    {
        ...
        $user = auth()->user();
        $user->notify(new AnimalFeedingNotification($yieldrecord));
        return redirect()->route('Yield.showyieldrecord', ['animal_id' => $animal_id])
    ...
    public function showyieldrecord($animal_id)
{
    ...
    return view('Yield.showyieldrecord', ['animal' => $animal, 'productStatistics' => $productStatistics, 'yieldrecords' => $yieldrecords,'user'=>$user]);
} catch (\Exception $e) {
    ...
    public function yieldrecord($animal_id)
    {
        try {
            ...
            return view('Yield.yieldrecord', ['animal' => $animal]);
        } catch (\Exception $e) {
            return redirect()->route('index')->with('error', 'An error occurred while displaying the yieldrecord form.');
        }
    }
...
public function edityieldrecord($animal_id, $yieldrecord_id)
{
    ...
    return view('Yield.yieldrecordedit', ['animal' => $animal, 'yieldrecord' => $yieldrecord]);
}
...
public function batchDelete(Request $request)
{
    try {
        ...
        yieldrecord::whereIn('id', $selectedIds)->delete();
