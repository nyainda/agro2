            ...
            <ol class="flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('treat.showtreatment', $animal->id) }}" class="text-sm font-medium text-gray-700 hover:text-indigo-600 dark:text-gray-300 dark:hover:text-indigo-400">{{ $animal->name }}</a>
                    </div>
                </li>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm font-serif   text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        ...
                        <tr id="sid{{$treatment->id}}" class="bg-white border-b font-serif  dark:bg-gray-800 dark:border-gray-700">

                            <td class="px-6 py-4">
                                <input type="checkbox" class=" dark:bg-gray-700 dark:text-gray-400" name="treatment_ids[]" value="{{ $treatment->id }}">
                            </td>
                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                ...
                                <td td class="px-6 py-4">
                                    <div class="flex space-x-2 items-center">
                                        <a href="{{ route('treatment.delete', ['animal_id' => $animal->id, 'treatment_id' => $treatment->id]) }}" class="inline-flex items-center px-2.5 py-1.5 bg-green-100 text-green-800 rounded-md hover:bg-green-200 transition duration-300 dark:bg-green-900 dark:text-green-300 dark:hover:bg-green-800">
                                        ...
                                        <div class="flex ">
                                            <a href="{{ route('treatment.delete', ['animal_id' => $animal->id, 'treatment_id' => $treatment->id]) }}" class="inline-flex items-center px-2.5 py-1.5 bg-red-100 text-red-800 rounded-md hover:bg-red-200 transition duration-300 dark:bg-red-900 dark:text-red-300 dark:hover:bg-red-800">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
$(document).ready(function() {
    $('#chkcheckAll').click(function() {
        $("input:checkbox[name='ids[]']").prop('checked', $(this).prop('checked'));
    });
        ...
        $.ajax({
           // url: "",
            ...
            data: {
                ...
                ids: allids  // Ensure UUIDs are correctly formatted as strings
            },
...
<script>
    function batchDelete() {
        var checkboxes = document.querySelectorAll('input[name="treatment_ids[]"]:checked');
        ...
        if (confirm("Are you sure you want to delete the selected items?")) {
            ...
            form.setAttribute('action', '{{ route("treatments.batchDelete") }}');
