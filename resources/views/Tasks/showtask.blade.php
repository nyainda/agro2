            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 @if($task->status == 'completed') line-through @endif">
                ...
                <td class="px-6 py-4">
                    ...
                    <div class="text-sm text-gray-500 mt-1">
                        @if($task->repeat_frequency && $task->end_repeat_date)
                            <span class="mr-2">Repeat every {{$task->repeat_frequency}}</span>
                            <span>Ends on {{$task->end_repeat_date}}</span>
                        @else
                            <span>No repeat</span>
                        @endif
                    </div>
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-between space-x-3 items-center">
                        <a href="{{ route('Tasks.taskedit', ['animal_id' => $animal->id, 'task_id' => $task->id]) }}" class="inline-flex items-center px-2.5 py-1.5 bg-green-100 text-green-800 rounded-md hover:bg-green-200 transition duration-300 dark:bg-green-900 dark:text-green-300 dark:hover:bg-green-800">
                        ...
                        <div class="flex space-x-3">
                            <a href="{{ route('task.delete', ['animal_id' => $animal->id, 'task_id' => $task->id]) }}" class="inline-flex items-center px-2.5 py-1.5 bg-red-100 text-red-800 rounded-md hover:bg-red-200 transition duration-300 dark:bg-red-900 dark:text-red-300 dark:hover:bg-red-800">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <form action="{{ route('Tasks.completeTask', ['animal_id' => $animal->id, 'task_id' => $task->id]) }}" method="post" class="inline">
                                @csrf
                                <button type="submit" class="inline-flex items-center px-2.5 py-1.5 bg-blue-100 text-blue-800 rounded-md hover:bg-blue-200 transition duration-300 dark:bg-blue-900 dark:text-blue-300 dark:hover:bg-blue-800">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
